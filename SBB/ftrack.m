...
% FTRACK This function creates a struct array with track info, important
%        for the TrainDy calculations
%
% INPUTS traccia:     Array with track info created in defTrack.m
%        scartamento: Gauge [m]. Input not used in the function and can be
%                     deleted [s!]
%
% OUTPUT strack:      Struct array with track info
...
% t:    type of track: 1 for straight,  2 for full curve, 3 for parabolic
...
strack = struct('t',0,'l',0,'c',0,'cv',0,'fi',0,'teta',0,'psi',0.5*pi, ...
    'alfa',0,'s',1,'x0',0,'y0',0,'C',[0 0],'B',0,'sx',+1,'sy',+1);
...
for k = 1:ntratti
    if traccia(k,6) == 1  % Straight stretch
        ...
        strack(k).psi  = psi0;                     % Tangent wrt XZ plane
    ...
    elseif traccia(k,6) == 2    % Full curve stretch
        ...
        R              = traccia(k,2);          % Curvature radius + right 
        strack(k).c    = 1/R;                   %                  - left
        strack(k).psi  = psi0;                  % Tangent wrt XZ plane
        strack(k).fi   = fi0;                   % Slope
        strack(k).teta = teta0;                 % Superelevation
        ...
        x0loc = abs(R)*cos(tetaloc); 
        y0loc = abs(R)*sin(tetaloc);
        ...
        vx0loc = abs(R)*cos(vtetaloc); 
        vy0loc = abs(R)*sin(vtetaloc);
    ...
    elseif traccia(k,6) == 3 && traccia(k,5) == 0    % Parabolic stretch (Clothoid, Cornu spiral)
        ...
        if tfc == 0
            ...
            strack(k).B = B;
            tfin = L/B;
            if traccia(k-1,2) == 0 % Previous track is a straight line
                segnoy = +1;
            ...
            xfin   = segnox*B*FresnelS; 
            yfin   = segnoy*B*FresnelC;
            taufin = 0.5*pi*tfin^2;

            sloc = linspace(0,L,Nt);
            tfin = sloc/B;
            [FresnelC,FresnelS] = fresnelCS(tfin);
            vxfin = segnox*B*FresnelS; 
            vyfin = segnoy*B*FresnelC;
            ...
            x = segnox*realsqrt(B2)*FresnelS; 
            y = segnoy*realsqrt(B2)*FresnelC;
    ...
    elseif traccia(k,5) ~= 0
        ...
        Rv = traccia(k,5);                      % Curvature radius + uphill
                                                %                  - downhill
