...
% PREL_COMP This function uses the buffer and draw gear parameters to
%           calculate piecewise polynomial approximations of their
%           characteristics. It also combines the characteristics of 
%           neighboring vehicles to compute the coupled information and
%           build the bgdg struct array
%
% INPUTS    nveicoli: Total number of vehicles
%           train:    Struct array with info about each vehicle
%
% OUTPUTS   bgdg:     Struct array with coupled info about buffers and draw
%                     gears. Size: 3 x (nveicoli-1)
%                     1st row: Right buffer
%                     2nd row: Draw  gear
%                     3rd row: Left  buffer
%           train:    Modified struct array


% Computing the number of points used in order to discretize the equivalent 
% force stroke characteristic of two consecutive buffers or draw gears

npxbgdg = 3;
for ii = 1:nveicoli;
    npxbgdg = max([npxbgdg size(train(ii).bgsf,1) size(train(ii).bgsr,1) ... 
        size(train(ii).dgsf,1) size(train(ii).dgsr,1)]);
end
npxbgdg = 2*npxbgdg;

% Initializations
zbgdg = zeros(1,npxbgdg);
% bgdg collects information about force-displacement coupling among right 
% buffers (first row), gears (second row) and left buffers (third row), for 
% each coupling
bgdg = struct('x',zbgdg,'fl',zbgdg,'fu',zbgdg,'pl',zbgdg,'pu',zbgdg,'c',1); % [s!] x field is probably outdated

% Piecewise approximation of the force-stroke characteristics for buffers 
% and draw gears
    ...
    for hh = 1:2
        if hh == 1 && train(ii).bgccr == train(ii+1).bgccf
            ...
            if size(bgdg(hh,ii).xl,2) ~= size(bgdg(hh,ii).xu,2)
                xuldiff = ['Loading and unloading curve of coupling ' ...
                    num2str(ii) 'have different points'];
                disp(xuldiff);
            end
        ...
        elseif hh == 2 && train(ii).dgccr == train(ii+1).dgccf
            ...
            % Error if a central coupler is coupled with a traditional coupling
...
for ii = 1:nveicoli-1
    ...
    % Polynomial used to manage load and unload
    ...
    if train(ii).gap >= 0
        % If the gap is positive the draw gear do not have any gap and it 
        % is assigned to the buffers to compiute correctly their compression  [s!] This comment needs to be rephrased
...
% Copy of the information of the buffing gear: it will represent the left 
% buffing gear behaviour
bgdg(3,:) = bgdg(1,:);
...
% EQUIV_BGDG This function computes the equivalent force stroke 
%            characteristic of buffers and draw gears in order to be stored
%            in the bgdg struct array
%
% INPUTS     npxbgdg: Number of points for the coupled characteristic
%            Pf:      Piecewise polynomial approximation front 
%            Pr:      Piecewise polynomial approximation rear 
%            s1:      Respective stroke values of the front gear
%            s2:      Respective stroke values of the rear  gear
% 
% OUTPUTS    F:       Vector with the force values of the coupled gear 
%                     characteristic [N]
%            vx:      The 3rd row of this array contains the stroke values
%                     of the coupled gear characteristic [m] (It's the
%                     sum of the two rows above, elementwise)
...
% FINDSOK This function [?]
% 
% INPUTS  s:      
%         ij:     Index to distinguish between front and rear gear
%         spoint: Starting point for the solution
%         f:      
%         pos:
%
% OUTPUT  sok:
...
% Unused function that can be removed or commented [s!]
...
% MODIF_FS Modify force-stroke characteristic in order to manage end of  
%          stroke issues
%
% INPUTS   s: Stroke values
%          f: Force  values
%
% OUTPUTS  s: Modified stroke values
%          f: Modified force  values
...
% PIECE_APPROX_BG_DG This function calculates the piecewise polynomial 
%                    approximation of every buffer and draw gear in the 
%                    train
%
% INPUTS             nveicoli: Total number of vehicles
%                    train:    Struct array with info about each vehicle         
%
% OUTPUT             train:    Modified struct array

for ii = 1:nveicoli
    
    % Force values from [kN] to [N] and stroke values from [mm] to [m]
    ...
    if train(ii).bgdf == 0  % If the damping coefficient is not given
        s = train(ii).bgsf; fu = train(ii).bgff(:,1); fl = train(ii).bgff(:,2);
        Pu = comp_poly_bgdgf(s,fu,'s',1); % Piecewise 1st order polynomial 
        Pl = comp_poly_bgdgf(s,fl,'s',1); % approximation
        
    else                    % Calculate the unload char. based on damping c
        s  = train(ii).bgsf; fl = train(ii).bgff(:,1); fu = (1-train(ii).bgdf)*fl;
        Pu = comp_poly_bgdgf(s,fu,'s',1);
        Pl = comp_poly_bgdgf(s,fl,'s',1);
    end
    train(ii).Pbgfl = Pl;   % Store the polynomial approximations to train
    train(ii).Pbgfu = Pu;   % struct array
...
% COMP_FORCE  This function computes the forces corresponding to the given 
%             strokes by evaluating the piecewise polynomial approximation.
%             NOTE that it is assumed that the stroke s contains the points
%             used to compute the vector of polynomial terms P
%
% INPUTS      s: Vector with stroke values
%             P: Polynomial terms of piecewise approximation
%
% OUTPUT      f: Computed force
...
% CHECKSLOPE This function ensures that there is a limited force
%            step between two consecutive points in the characteristic [?]
%
% INPUTS     x:     Old x values
%            y:     Old y values
%
% OUTPUTS    newx:  New x values
%            newy:  New y values
...
if nx >= 3
    ...
    if size(yy,2) > 1 && sum(yy(:,2)) > 0
        ...
        elseif sum(xx(:,2)-xx(:,1)) > 1
            % Forces are computed in different points: it is necessary to 
            % create a unique x-vector
            x = [xx(:,1);xx(:,2)];
            x = sort(x);
            d = diff(x);
            x(d == 0) = [];
...
% SAMEABSCISSA It is necessary that the loading and un-loading curve have 
%              the same abscissa. Moreover it is necessary that the loading 
%              curve is above the un-loading curve. Lastly, it is necessary 
%              to extend the force-stroke curve in order to avoid problems 
%              during the integration
%
% INPUTS       bgdg:  Struct array with coupled info about buffers and draw
%                     gears
%              ii:    Column index of bgdg struct array (1 to nveicoli)
%              ind:   Row index of bgdg struct array    (1 or 2, since
%                     the third row is just a copy of the first one)
%              train: Struct array with info about each vehicle
%
% OUTPUT       bgsg:  Modified struct array
...
% CHECKLOAD_UNLOAD This function changes, if necessary, the un-loading 
%                  curve so that it is below the loading curve
%
% INPUTS           xl: Vector with strokes of load   curve
%                  fl: Vector with forces  of load   curve
%                  xu: Vector with strokes of unload curve
%                  fu: Vector with forces  of unload curve
%
% OUTPUTS          xu: Modified stroke vector of unload curve
%                  fu: Modified force  vector of unload curve
...
% COMP_PRELOAD This function is used in order to compute the preload effect 
%              XR and XT in buffer and draw gear in case of preload 
%              displacement deltaG. (gap field in bgdg)
%
% INPUTS       Vxresp:       Stroke vector of buffer in load curve
%              Vxtir:        Stroke vectot of draw gear in load curve
%              polyrespL:    Piecewise polynomial approximation of the
%                            load curve for buffers
%              polyrespUL:   Piecewise polynomial approximation of the
%                            unload curve for buffers
%              polytirL:     Piecewise polynomial approximation of the
%                            load curve for draw gears
%              polytirUL:    Piecewise polynomial approximation of the
%                            unload curve for draw gears
%              deltaG:       
%              PolyRaccResp: Polynomial used for determining the coupling 
%                            force of a buffer when the approaching speed 
%                            is not greater than vpl or the distancing
%                            speed is not greater than vpu. It is written 
%                            as c(v) in user guide p.72
%              PolyRaccTir:  Accordingly for draw gears
%
% OUTPUTS      XR: (?)
%              XL: (?)
...
% VISCOUS_DAMPING This function combines the viscous damping coefficients
%                 of two neighboring vehicles
%
% INPUTS          bgdg:  Struct array with coupled info about buffers and
%                        draw gears
%                 hh:    Index to distinguish between buffer and draw gear 
%                 ii:    Index of current vehicle. ii and ii+1 vehicle
%                        information are coupled
%                 train: Struct array with info about each vehicle
%
% OUTPUTS         bgdg:  Modified struct array
