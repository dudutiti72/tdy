...
while jm <= nManv
    ...
    if isempty(swcf) && sum(pfin) > 0
        ...
        [Cfq,grapi,prpi,ptarget,segnof,Sut,t1] = initialManvBP(CA,CVactv, ...
            DBVdata,dt,isservice,nCV,pCF,pfin,Pn,prpi,typeDBV);
...
if isempty(swcf)
    ...
    %         Therefore, dy contains the velocities and accelerations
        ...
        % Initializations of loco forces
                                                                            % [s!] brake_eldyn and trac_eldyn do need Feldyn and Ftrac as inputs since
                                                                            %      the calculations are not based on previous force values
        Feldyn = znl;
        Ftrac  = znl;
        if any(eldyn)
                ...
                case 'done'                           % odeplot([],[],'done')
                    ...
                    if isempty(swcf)
                        if jm == nManv
                            ...
                            % the simulation.
                            input('It seems some informations is missing. Please fix me!');
                            pBCt = pBCtpSR;
