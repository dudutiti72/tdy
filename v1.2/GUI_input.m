...
[Dcoll,epsilon,kcoll,Lcoll,nloco,nwago,nveicoli,P1,pathGUI,ploco, ...        
    pwago,sep,solver,SR,Tamb,trainGUI] = configuration(appf,fsl,nzug,trainGUI);   
...
% Filling "train" with information about buffers and draw gears
...
function [Dcoll,epsilon,kcoll,Lcoll,nloco,nwago,nveicoli,P1,pathGUI,ploco,pwago,sep,solver,SR,Tamb,...
...
trainGUI = train_conf_fric(trainGUI,'$EXT_BLOCK_FRICTION_LAWS','frictLaw=',lsep,nf,nveicoli,sep,'frlaw');
...
% Other outputs
nwago = 0; nloco = 0; ploco = zeros(1,nveicoli); pwago = ploco;
...
function trainGUI = train_conf_fric(trainGUI,dol,field,lsep,nf,nveicoli,sep,trainfield)
